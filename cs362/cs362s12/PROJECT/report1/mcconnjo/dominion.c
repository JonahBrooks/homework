...
int initializeGame(int numPlayers, int kingdomCards[10], int randomSeed,
  ...
  ///////////////////////////////
    ...
    //set embargo s to 0 for all supply piles
...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  if(supplyPos < 0){
	...
	//supplyPos is a negative index
	//TODO
  }
  if(supplyPos > treasure_map){
	//error index out of bounds
	//supplyPos > than maxBound
	//TODO
  }
  // THIS IMPLEMENTATION IS NOT WORKING, BUT LETS YOU PLAY.  FIX IT
  if(state->numBuys < 1){
	//error cannot buy anymore
	//TODO
	    return 0;
  }

  if(state->supplyCount[supplyPos] < 1){
	//error there is no more of that card
	//TODO
	    return 0;
  }

  if(state->coins - getCost(supplyPos) < 0){
      return 0;
  }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case adventurer:
      ...
      do{
        //draw card, if card is treasure then add to hand and increment numTreasure
        if(state->deckCount[currentPlayer] < 1){
            shuffle(currentPlayer, state);
        }

        tempCard = state->deck[currentPlayer][state->deckCount[currentPlayer]--];
        if(tempCard == copper || tempCard == silver || tempCard == gold){
            numTreasure++;
            state->hand[currentPlayer][state->handCount[currentPlayer]++] = tempCard;
        }
        else{
            temphand[numOther++] = tempCard;
        }
        state->deckCount[currentPlayer]--;
      }while(numTreasure < 2);

      //place temphand into discard pile
      ...
      if( state->supplyCount[choice1] == -1 )
	{
      ...
      //add embargo  to selected supply pile
