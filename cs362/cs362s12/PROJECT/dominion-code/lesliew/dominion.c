...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  int owed = getCost(supplyPos);
  int who = whoseTurn(state);
  updateCoins(who, state , 0);
  if(state->coins > owed){
	state->numBuys--;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
		...
		i = 0;
		while(count < 2) {
			if (state->deck[whoseTurn(state)][i] == copper){
				gainCard(copper, state, 2, whoseTurn(state));
				count++;
			}
			if (state->deck[whoseTurn(state)][i] == silver){
				gainCard(silver, state, 2, whoseTurn(state));
				count++;
			}	
			if (state->deck[whoseTurn(state)][i] == gold){
				gainCard(gold, state, 2, whoseTurn(state));
				count++;
			}
			else{
				gainCard(state->deck[whoseTurn(state)][i], state, 0, whoseTurn(state));
			}
			i++;
		}
