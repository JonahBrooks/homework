...
int initializeGame(int numPlayers, int kingdomCards[10], int randomSeed,
  ...
  debug("in initializeGame, about to set up random # gen");
					
  //set up random number generator
  SelectStream(1);
  PutSeed((long)randomSeed);
  
  debug("successfully set up random # gen, about to check numplayers and maybe fail");
  ...
  debug("numplayers seemed okay; setting it into state");

  //set number of players
  state->numPlayers = numPlayers;

  debug("now to check that kingdom cards are different");
...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  //is there a card supplyPos available?
  ...
  //put it in the player's discard pile
  state->discard[who][state->discardCount[who]] = supplyPos;
  state->discardCount[who]++;

  //TODO: Take away the money they spent
  //TODO: make sure the turn ends if the player is out of buys
