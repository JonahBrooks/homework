...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  if(state->numBuys >= 1 && state->supplyCount[supplyPos] > 0 && state->coins >= getCost(supplyPos)){
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
      ...
      i = 0;
	  k = 0;
	  j = state->deckCount[currentPlayer] - 1;
      while(i < 2 && k < 2){
		...
		if(state->deck[currentPlayer][j] == gold 
          || state->deck[currentPlayer][j] == silver 
          || state->deck[currentPlayer][j] == copper){
		  
		  state->hand[currentPlayer][state->handCount[currentPlayer]] = state->deck[currentPlayer][j];
		  state->handCount[currentPlayer]++;
		  state->deckCount[currentPlayer]--;
		  i++;
		}
	    else {
		  state->discard[currentPlayer][state->discardCount[currentPlayer]] = state->deck[currentPlayer][j];
