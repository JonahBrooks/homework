...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
	  ...
	  // Not exactly right, I need to make a temporary storage and make sure to shuffle the deck
	  while (treasureCards < 2)
	  {
		drawCard(currentPlayer, state);
		if (state->hand[currentPlayer][state->handCount[currentPlayer]-1] == copper || silver || gold)
		{
			treasureCards++;
		}
		else
