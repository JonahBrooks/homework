...
int playCard(int handPos, int choice1, int choice2, int choice3, struct gameState *state)
{
	...
	state->numActions--;
	/* When out of actions, move to buy phase */
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	case adventurer:
		while (treas_drawn < 2)
		{
			...
			card_drawn = handCard(numHandCards(state) - 1, state);
			if (card_drawn == 4 || card_drawn == 5 || card_drawn == 6)
			{
				treas_drawn++;
			}
			else
