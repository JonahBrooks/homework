...
int buyCard(int supplyPos, struct gameState *state) {
	...
	if (DEBUG){
		...
		printf("%d left in supply and %d coins left.\n", state->supplyCount[supplyPos],
			state->coins);
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	switch( card )
	{
		case adventurer:
		{
			...
			// Reveal cards until two treasure cards are revealed
			// Keep the treasure and discard the rest
			int numTreasure = 0;
			int initialHand = state->handCount[currentPlayer];
			while (numTreasure < 2){
			...
			while (1){
				int currentCard = state->hand[currentPlayer][initialHand];
				if (currentCard >= copper && currentCard <= gold){
					initialHand += 1;
				} else if (initialHand >= state->handCount[currentPlayer]) {
					break;
				} else {
					if (DEBUG) {
						printf("Player %d discarded #%d\n", currentPlayer, currentCard);
					}
					discardCard(initialHand, currentPlayer, state, 0);
				}
			}
