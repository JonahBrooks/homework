...
if(supplyCount(supplyPos,state)==0){
	printf("All out of that card");
	return -1;

}
else if(state->coins<getCost(supplyPos))
{
	printf("Can't buy card");
	return -1;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
	...
	else if(state->hand[currentPlayer][state->handCount[currentPlayer]-1]==silver){
        i++;
}
	else if(state->hand[currentPlayer][state->handCount[currentPlayer]-1]==copper){
        i++;
}
	else{
