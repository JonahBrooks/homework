...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  // check if the player has enough buy phase 
  if(state->numBuys < 1)
	  return -1;
  // check if the player has enugh coins to buy the card 
  if (getCost(supplyPos)> state->coins)
	  return -1; // the player does not have enough conis to buy the card
  // check if the card has not yet been sold out
	if (supplyCount(supplyPos, state)==0)
		return -1;// the card requested is sold out
  
	// buy the card and take off costing coins from the player coins
	else{
	state->phase++;
	printf("Player:%d has %d coins \n",who ,state->coins);
	printf("Card Requested costs %d coins \n",getCost(supplyPos));
	state->coins -= getCost(supplyPos);
	printf("Player:%d has %d coins left\n",who,state->coins);
    ...
    case adventurer:
      ...
      // YOU MUST IMPLEMENT ADVENTURER YOURSELF
		...
		*/
		...
		// loop to draw card until 2 treasure cards found  
		printf("*************Started with %d Coins**********\n",state->coins);
		while(treasure_card!=0){

			printf("Reveal one card current conis %d\n",state->coins);
			temp_handPos = numHandCards(state) - 1; 
			temp_card = handCard(temp_handPos,state); 
			// check if the handed card is a treasure card or not
			if (temp_card == 4 ||temp_card==5 || temp_card==6){
				drawCard(currentPlayer, state);
				treasure_card--;
				printf("a treasure card has been found it is %d\n", temp_card);
				printf("------------- NOW %d Coins ------------\n",state->coins);
			}
			else // if it is not then discard 
				discardCard(temp_handPos, currentPlayer, state, 0);
		}
