...
int discardDeckCard(int deckPos, int currentPlayer, struct gameState *state);
int getCardValue(int card);

void cardNumToNameTest(int card, char *name){
  switch(card){
...
int buyCard(int supplyPos, struct gameState *state) {
	...
	int cost = 0;
	char cardName[30] = "";
	cardNumToNameTest(supplyPos, cardName);

	if(LOG) {
		printf("\tCard to buy: %s (supplyPos = %i)\n", cardName, supplyPos);
	}
	// *********** Buy phase ***************
	// Place the selected card (from the supply
	// pile) in the discard pile
	// ************************************
	...
	if(LOG){
		...
		printf("\tcoins in hand = %i\n", state->coins); 
	}

	// Check whose turn is
	int who = state->whoseTurn;
	int choice1 = supplyPos;
	int currentPlayer = who;
	int i;
	...
	if(LOG){
		...
		for(i = 0; i < state->handCount[currentPlayer]; i++){
			printf("\t\thand[%i][%i] = %i (%i coins)\n", 
			       currentPlayer, i,
			       state->hand[currentPlayer][i], 
			       getCardValue(state->hand[currentPlayer][i]));
		}
		// Print discard pile
		printf("\tCards in discard pile (count=%i):\n", state->discardCount[currentPlayer]);
		...
		/*
		// Deck pile
		printf("\tCards in deck pile now (count=%i):\n", state->deckCount[currentPlayer]);
	...
	if(supplyCount(choice1, state) <= 0) {
		if(LOG) {
			...
			printf("\tCards Left: %d, card = %d\n", supplyCount(choice1, state), choice1);
		}
		return -1;
		...
		if(LOG) {
			printf("\tNot enough buys found\n");
		}
		return -1;
	} else {
		
		if(LOG) {
			printf("\tDeck Count: %d (before buying)\n", 
			       state->deckCount[currentPlayer]);
			printf("\tBuying card %s (%d)\n", cardName, choice1);
		}
	
		gainCard(choice1, state, 0, currentPlayer);//Gain the card
		state->numBuys--;
		if(LOG) {
			printf("\tDeck Count: %d (after buying)\n", 
			       state->deckCount[currentPlayer]);
		}
	}
...
int drawCard(int player, struct gameState *state)
{
	if(state->deckCount[player] <= 0){//Deck is empty
		...
		if(DEBUG){//Debug statements
			printf("drawCard: Deck count now: %d\n", state->deckCount[player]);
		}
		...
		if(DEBUG){//Debug statements
			printf("drawCard: Current hand count: %d\n", count);
		}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	int adventurerRevealedCards[2] = {-1, -1};
	...
	switch( card ) 
		...
		case adventurer:
			{
				// YOU MUST IMPLEMENT ADVENTURER YOURSELF

				/* Note: Adventurer – If you have to shuffle in the middle, shuffle. Don't
				...
				*/
				// 1. Reveal cards from your deck until you reveal 2 Treasure cards
				if (DEBUG) {
					printf("\tUser %i is about to play advendurer card.\n", currentPlayer);
					// Print discard pile
					int i;
					printf("\tCards in discard pile now (count=%i):\n", state->discardCount[currentPlayer]);
					for(i = 0; i < state->discardCount[currentPlayer]; i++){
						printf("\t\tdiscard[%i][%i] = %i\n", 
						       currentPlayer, i,
						       state->discard[currentPlayer][i]);
				...
				int countRevealed = 0;
				int deckCounter = state->deckCount[currentPlayer];//Create holder for the deck count
				...
				if(deckCounter == 0){
					...
					deckCounter = state->deckCount[currentPlayer];//update deck counter
				} 
				
				for (i = deckCounter - 1; i >= 0 ; i--) {
					if (DEBUG) {
						printf("\tRevealing card %i, (index = %i)\n", state->deck[currentPlayer][i], i);
					}

					if (state->deck[currentPlayer][i] == copper ||
					    state->deck[currentPlayer][i] == silver ||
					    state->deck[currentPlayer][i] == gold) {
						adventurerRevealedCards[countRevealed] = state->deck[currentPlayer][i];
						countRevealed++;
						if (countRevealed == 2) {
				...
				if (DEBUG) {
					int j;
					printf("\tTreasure Cards revealed:\n");
					for (j = 0; j < countRevealed; j++) {
						printf("\t\tTreasure card: %i (coins = %i)\n", 
						       adventurerRevealedCards[j], 
						       getCardValue(adventurerRevealedCards[j]));
					}
				}


				// 2. Put these Treasure cards into your hand
				if (countRevealed == 2) {
					state->hand[currentPlayer][state->handCount[currentPlayer]] = adventurerRevealedCards[0];
					state->handCount[currentPlayer]++;
					state->hand[currentPlayer][state->handCount[currentPlayer]] = adventurerRevealedCards[1];
					state->handCount[currentPlayer]++;
				}
				...
				if(DEBUG){
					int k;
					// Print hand cards
					printf("\tCards in hand for buy phase:\n");
					for(k = 0; k < state->handCount[currentPlayer]; k++){
						printf("\t\thand[%i][%i] = %i (%i coins)\n", 
						       currentPlayer, k,
						       state->hand[currentPlayer][k], 
						       getCardValue(state->hand[currentPlayer][k]));
					}
										// Print discard pile
					printf("\tCards in discard pile now (count=%i):\n", state->discardCount[currentPlayer]);
...
{
	//set played card to -1
	state->deck[currentPlayer][deckPos] = -1;
	
	//remove card from player's deck
	if( deckPos == (state->deckCount[currentPlayer] - 1) ) 	
		//last card in deck array is played
		{
			//reduce number of cards in deck
			state->deckCount[currentPlayer]--;
		}
	else if( state->deckCount[currentPlayer] == 1 ) //only one card in deck
		{
			//reduce number of cards in deck
			state->deckCount[currentPlayer]--;
		}
		...
		{
			//replace discarded card with last card in deck
			state->deck[currentPlayer][deckPos] = 
				state->deck[currentPlayer][ (state->deckCount[currentPlayer] - 1)];

			//set last card to -1
			state->deck[currentPlayer][state->deckCount[currentPlayer] - 1] = -1;

			//reduce number of cards in dekc
			state->deckCount[currentPlayer]--;
		}
