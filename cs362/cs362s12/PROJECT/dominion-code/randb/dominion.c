...
int buyCard(int supplyPos, struct gameState *state)
{
	...
	{
		return -1;//has the coin?
	}
	else
	{
		state->coins -= getCost(supplyPos);//get card
		state->discard[who][state->discardCount[who]] = supplyPos;
		state->discardCount[who]++;
	}
	int end = state->coins;
	
	updateCoins(whoseTurn(state),state,(end-begin));
...
int drawCard(int player, struct gameState *state)
{
	if(state->deckCount[player] <= 0) //Deck is empty
	{
		...
		//Shufffffffffffffffffffle the deck
		shuffle(player, state);//Shuffle the deck up and make it so that we can draw

		if(DEBUG) //Debug statements
		{
			printf("DC(%d)", state->deckCount[player]);
		}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	case adventurer:
		...
		while((treasures < 2) && (drawCard(currentPlayer, state) != -1))
		{
			cur_card = handCard((numHandCards(state) - 1), state);
			if (cur_card >= 4  || cur_card <= 6)
			{
				treasures++;
			}
			else
			{
				discardCard((numHandCards(state) - 1), currentPlayer, state, 0);
			}
		}
