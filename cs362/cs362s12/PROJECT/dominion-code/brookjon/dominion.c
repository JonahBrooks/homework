...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  // Nothing is ever changing the phase, but if it did:
  // if (state->phase != 1) { } else
  if (state->numBuys < 1) {
    //printf("No more buys remaining\n");
  } else if (state->supplyCount[supplyPos] < 1){
    //printf("Supply exhausted\n");  
  } else if (getCost(supplyPos) > state->coins){ 
    //printf("Not enough coins\n");
  } else {
    (state->numBuys)--;  
    //// something with money... or... do you even discard money for each purchase?
    //// Maybe I'll leave this exciting little design flaw -- I mean bug -- as 
    //// an exercise for the reader XD
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
      ...
      // Draw cards into your hand, discarding any that are not treasure
      // Stop when you've gained two treasure, or you've gone through the
      // deck at least one full time (shuffled it twice)
      while(cards_added < 2 && shuffles < 2){
        if(state->deckCount[currentPlayer] <= 1) {
          shuffles++;
        }
        if(drawCard(currentPlayer, state)) {
          new_card = state->hand[currentPlayer][state->handCount[currentPlayer]-1];
