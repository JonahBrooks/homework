...
int buyCard(int supplyPos, struct gameState *state) {
	...
	//updateCoins(currentPlayer, state, 0); Causes seg fault?
	if(state->coins <=0){
		if(DEBUG){
    	...
    	if(supplyCount(supplyPos, state) <= 0){
			...
			state->phase = 1;
			gainCard(supplyPos, state, 0, currentPlayer);//Gain the card and put in discard pil. - as per rules.
			state->numBuys--;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
     ...
     */
    while(TreasureCount < 2 && Shuffled < 1){
		// We didn't find enough treasure so shuffle. We can only do this once.
		if(state->deckCount[currentPlayer] < 1){
			shuffle(currentPlayer, state);
			++Shuffled;
		}
		//Set the current card to the top card on the deck.
		CurrentCard = state->deck[currentPlayer][state->deckCount[currentPlayer]];
		
		/*
		 * Check the top card to see if it is a treasure card.
		 *If a treasure card add it to the players hand and increase TreasureCount.
		 * Otherwise, throw the current card into the temporary holder.
		 */
		if(CurrentCard == copper || CurrentCard == silver || CurrentCard == gold){
			TreasureCount++;
			state->hand[currentPlayer][state->handCount[currentPlayer]++] = CurrentCard;
		}else{
			temphand[Count++] = CurrentCard;
		}
		//Whether it is a treasure card or not we need to take the card out of the deck.
		// Decrease the count.
		state->deckCount[currentPlayer]--;
	}
		  
	//Transfer the cards that we found not to be treasure to the discard.
	for(i = 0; i < Count; i++){
    ...
    case sea_hag:
      for(i = 0; i < state->numPlayers; i++){
			if(i != currentPlayer){
				...
				state->deck[i][state->deckCount[i]-1] = 0;
			}
      }
	  discardCard(handPos, currentPlayer, state, 0);
      return 0;
