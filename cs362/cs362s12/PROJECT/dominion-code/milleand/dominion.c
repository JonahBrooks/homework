...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  int count = 0;  //number of cards added to the hand by adventurer
  int drawn_card = -1;
    ...
    case adventurer:
	    ...
	    while(count < 2){
		    if(drawCard(currentPlayer, state) == 0){
			    index = state->handCount[currentPlayer] - 1;
			    drawn_card = state->hand[currentPlayer][index];
			    if(drawn_card == copper || drawn_card == silver || drawn_card == gold){
				    count++;
			    }
			    else{
				    index = state->handCount[currentPlayer]-1;
