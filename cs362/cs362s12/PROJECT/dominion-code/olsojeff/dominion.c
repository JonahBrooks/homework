...
const char* card_names[] = {"curse", "estate", "duchy", "provine", "copper", "silver", "gold", "adventurer", "council_room", "feast",
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
      ...
      for (i=state->deckCount[who];i>=0;i--){
        printf("Adventurer revealing %s.", card_names[state->deck[who][i]]);
        if (state->deck[who][i] == copper || state->deck[who][i] == silver || state->deck[who][i] == gold){
          gainCard(state->deck[who][i], state, 1, who);
        }
        else{
          state->discard[who][state->discardCount[who]] = state->deck[who][i];
          state->deckCount[who] -= 1;
          state->discardCount[who] += 1;
        }
      }
