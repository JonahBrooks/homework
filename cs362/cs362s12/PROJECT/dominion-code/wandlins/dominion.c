...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  int treasure = 0, temp, notTreasure = 0;
  ...
  switch( card ) 
    ...
    case adventurer:
      do{
		  ...
		  temp = state->deckCount[currentPlayer]--;
		  state->deckCount[currentPlayer]--;
		  if(temp==gold||temp==silver||temp==copper) {
			  state->hand[currentPlayer][state->handCount[currentPlayer]++] = temp;
			  treasure++;
		  }
		  else{
			  notTreasure++;
			  temphand[notTreasure] = temp;
		  }
	  }while(treasure<2)

		for(i=0; i < notTreasure; i++) {
