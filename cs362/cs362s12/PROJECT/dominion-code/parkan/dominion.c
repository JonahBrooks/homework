...
int buyCard(int supplyPos, struct gameState *state) {
	...
	if(state->numBuys == 0){
		printf("You don't have any buys");
		return -1;
	}
	
	if(supplyCount(supplyPos, state) > 0){
		if(state->coins < getCost(supplyPos)){
			printf("Invalid amount of coins");
			return -1;
		}
		else{
			gainCard(supplyPos, state, 0, currentPlayer);
			state->coins = state->coins - getCost(supplyPos);
			state->numBuys = state->numBuys - 1;
		}
    }
	else{
        printf("There is no more of that card");
		return -1;
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
    	//draw cards until you have 2 teasure cards, discard the rest
    	i = 0;
    	while(i < 2){
		drawCard(currentPlayer, state);
		j = state->handCount[currentPlayer];
        	if(state->hand[currentPlayer][j - 1] == copper || state->hand[currentPlayer][j - 1] == silver || state->hand[currentPlayer][j - 1]){
                	i++;
		}
		else{
			discardCard(j - 1, currentPlayer, state, 0);
		}
