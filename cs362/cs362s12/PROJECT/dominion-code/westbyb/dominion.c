...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
	...
	temp = state->deckCount[currentPlayer][state->deckCount[currentPlayer]--];
	// if the drawn card is a treaure card, then add it to the hand and increment numTreasure
	if(temp == copper || temp == silver || temp == gold){
	  numTreasure++;
	  state->hand[currentPlayer][state->handCount[currentPlayer]++] = temp;
	}
	//else, add temp card into temphand
	else{
	  temphand[numOther++] = temp;
	}
	state->deckCount[currentPlayer]--;
      }
      
      // move temphand into the discard pile
