...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  if(state->numBuys == 0)
  {
	printf("You are out of buys for this turn.\n");
	return -1;
  }
  if(supplyCount(supplyPos, state) > 0)
  {
  	...
  	{
		printf("You don't have the money for that card.\n");
		return -1;
  	} else {
		gainCard(supplyPos, state, 0, whoseTurn(state));
		state->coins -= getCost(supplyPos);
		state->numBuys -= 1;
		return 0;
  	}
  } else {
	printf("That card is out of stock.\n");
	return -1;
  }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
	...
	if(state->hand[currentPlayer][state->handCount[currentPlayer]-1] == gold ||
	   ...
	   state->hand[currentPlayer][state->handCount[currentPlayer]-1] == copper) {
		i++;
	} else {
