...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    {
    case adventurer:
       //loop until 2 treasure cards found or both deck and discard piles are empty
      while(treasureCardCount!=2 || (state->deckCount[currentPlayer] <= 0 && state->discardCount[currentPlayer] <= 0)){
          if(state->deckCount[currentPlayer]>0){
              state->playedCards[state->playedCardCount] = state->deck[currentPlayer][handPos];
              //check the card from the deck to see if it's a treasure card
              if(state->deck[currentPlayer][handPos] == copper){
                  treasureCardCount++;
                  state->handCount[currentPlayer]++;
                  state->coins += state->coins + 1;
              }
              else if(state->deck[currentPlayer][handPos] == silver || state->deck[currentPlayer][handPos] == gold){
                  treasureCardCount++;
                  state->handCount[currentPlayer]++;
                  state->coins += state->coins + 2;
              }
              else if(state->deck[currentPlayer][handPos] == gold){
                  treasureCardCount++;
                  state->handCount[currentPlayer]++;
              ...
              //Shufffle the deck
              shuffle(currentPlayer, state);//Shuffle the deck up and make it so that we can draw
