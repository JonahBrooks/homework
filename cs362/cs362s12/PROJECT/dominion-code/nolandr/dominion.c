...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  if(state->numBuys < 1){
	...
	for(i=0; i<index; i++){
		if(state->hand[who][i] == 4){
			state->coins = state->coins+1;
		}
		else if(state->hand[who][i] == 5){
			state->coins = state->coins+2;
		}
		else if(state->hand[who][i] == 6){
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
	  ...
	  i=0; //Treasure Counter
	  j=0; //Temporary Hand Index
	  
	  while(i<2){
		if(state->deckCount[currentPlayer] == 0 && state->discardCount[currentPlayer] == 0){
			if(j != 0){
				for(k=0; k<j; k++){
					state->discard[currentPlayer][k] = temphand[k];
				}
			}
			return 0;
		}
		else{
			index = state->handCount[currentPlayer];
			drawCard(currentPlayer, state);
			if(state->hand[currentPlayer][index] != 4 && state->hand[currentPlayer][index] != 5 && state->hand[currentPlayer][index] != 6){
				i++;
			}
			else{
				temphand[j] = state->hand[currentPlayer][index];
				state->hand[currentPlayer][index] = -1;
