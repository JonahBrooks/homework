...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  int active_player = state->whoseTurn;
  
  // Check to see if you have a buy phase for buying a card
  if(state->numBuys < 1){
	//printf("You do not have enough buys \n");
    return -1;
  }
  
  // Check to see if the card is worth less than the gold you have
  if(state->coins < getCost(supplyPos)){
	//printf("You cannot afford this card \n");
	//printf("Your gold: %d, Card cost: %d \n", state->coins, getCost(supplyPos));
	return -1;
  }
  
  // Check to see if the card is still in the game
  if(supplyCount(supplyPos, state) < 1){
	//printf("There are no more cards in that pile \n");
	return -1;
  }
  
  gainCard(supplyPos, state, 0, active_player);
  state->coins = state->coins - getCost(supplyPos);
  state->numBuys--;
  state->discard[active_player][state->discardCount[active_player]] = supplyPos;
