...
int initializeGame(int numPlayers, int kingdomCards[10], int randomSeed,
	...
	bzero((void *)state, sizeof(struct gameState));
...
int buyCard(int supplyPos, struct gameState *state) {
	...
	//Check if player has enough buys and coins and that there is a card to buy
	if(state->numBuys > 0 && state->supplyCount[supplyPos] > 0 
						  && state->coins >= getCost(supplyPos)){
		
		state->phase = 1; // Enter Buy Phase
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
	  k = 0;
      i = -1; // To Search through Deck
	  j = -1; // To Store First Treasure Found
	  while(state->deckCount[currentPlayer] > 0){
		...
		if( i == copper || i == silver || i == gold){
			...
			state->playedCards[state->playedCardCount] = i;
			state->playedCardCount++;
			i = -1;
		}
	  
		if(state->deckCount[currentPlayer]-- == 0 && k == 0){
	  ...
	  if(j!=-1){
		state->hand[currentPlayer][state->handCount[currentPlayer]] = j;
		state->handCount[currentPlayer]++;
	  }
	...
	while(card_not_discarded){
	  if(state->hand[currentPlayer][p] == estate){//Found an estate card!
	    ...
	    for(p = p; p < state->handCount[currentPlayer]; p++){
