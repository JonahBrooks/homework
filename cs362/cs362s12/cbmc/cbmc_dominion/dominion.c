...
int shuffle(int player, struct gameState *state) {
  ...
  while (state->deckCount[player] > 0) {
    card = 0; // BRAIN SURGERY WITH CHAINSAW
...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  // THIS IMPLEMENTATION IS NOT WORKING, BUT LETS YOU PLAY.  FIX IT
  int gainRet;

  int who = state->whoseTurn;

  int price = getCost(supplyPos);

  printf("Player is buying a card costing %d, and has %d cash\n",price, state->coins);
  ...
  if(price <= state->coins){
    gainRet = gainCard(supplyPos, state, 0 , who);
    if(gainRet == -1){
      printf("The requested card is not available.");
      return -1;
    }else{
  ...
  if (DEBUG){
    printf("Bought #%d for %d coins.\n", supplyPos, getCost(supplyPos));
    printf("%d #%d left in supply and %d coins left.\n", state->supplyCount[supplyPos], supplyPos, state->coins);
  }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
      ...
      backupSize = state->handCount[currentPlayer];
	...
	state->hand[currentPlayer][i] = -1;//Set to nothing
      ...
      while (adventurerTreasureFound < 2){
        drawCard(currentPlayer, state);
        int currentCard = state->hand[currentPlayer][state->handCount[currentPlayer]-1];
	if (DEBUG) {
	  printf("Player %d drew #%d\n", currentPlayer, currentCard);
	}
	if (currentCard >= copper && currentCard <= gold){
	  adventurerTreasureFound += 1;
          backupSize += 1; //These next 2 lines move the treasure to the backup hand and remove it from the hand
          temphand[backupSize-1] = currentCard;
          state->handCount[currentPlayer] -= 1;
	}
      }

      //Restore Backup Hand with new treasures
      state->handCount[currentPlayer] = backupSize;
      for(i = 0; i <= state->handCount[currentPlayer]; i++){
      ...
      for(i = 0; i < state->handCount[currentPlayer]; i++)
	{
	  if(state->hand[currentPlayer][i] == j)
	    {
	      discardCard(i, currentPlayer, state, 1);	//changed trash flag from 0 to 1		
	      break;
	    }
	}
